...
export class Track {
  readonly cx = TRACK.centerX;
  readonly cy = TRACK.centerY;
  readonly L = TRACK.straightLength;
  readonly R = TRACK.curveRadius;
  readonly C = Math.PI * this.R;
  readonly perimeter = 2 * this.L + 2 * this.C;
  ...
  sample(progress: number): TrackPoint {
    ...
    if (p < L) return { x: cx - L / 2 + p, y: cy + R, tangent: 0 };
    if (p < L + C) {
      const local = p - L;
